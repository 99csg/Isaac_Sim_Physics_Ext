...
from .Epson_C8_A901_IKSolver import EpsonControl
...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id: str):
        ...
        self.ik_solver = EpsonControl()
        self.ik_solver.on_init()
        
        # Connect IK Solver to UI
        self.ui_builder.set_ik_solver(self.ik_solver)
    ...
    def _on_timeline_event(self, event):
        if event.type == int(omni.timeline.TimelineEventType.PLAY):
            ...
            # Notify IK Solver of play event
            if hasattr(self, 'ik_solver') and self.ik_solver:
                self.ik_solver.on_play()
        elif event.type == int(omni.timeline.TimelineEventType.STOP):
            self._physx_subscription = None
            # Notify IK Solver of stop event
            if hasattr(self, 'ik_solver') and self.ik_solver:
                self.ik_solver.on_stop()
        elif event.type == int(omni.timeline.TimelineEventType.PAUSE):
            # Notify IK Solver of pause event
            if hasattr(self, 'ik_solver') and self.ik_solver:
                self.ik_solver.on_pause()

        self.ui_builder.on_timeline_event(event)

    def _on_physics_step(self, step):
        # Update IK Solver on each physics step
        if hasattr(self, 'ik_solver') and self.ik_solver:
            # Get proper delta_time from timeline
            current_time = self._timeline.get_current_time()
            delta_time = self._timeline.get_time_codes_per_seconds() / 60.0  # Approximate delta
            self.ik_solver.on_update(current_time, delta_time)
